    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (button == panel.getQuitButton()) {
          ...
          panel.getCanvas().repaint();
            ...
            int index = panel.getCanvas().getQuestionIndex();
            if(index < 0){
             JOptionPane.showMessageDialog(panel.getWindow(),"Question is not presented yet");
            return;
            }
            ...
            if(QuestionBank.database.get(index).checkAnswer(response)){
                message = response + " :Right answer!";

            }
            else {
                message = response + " :Wrong answer. Better luck next time";
            }
